...
void setup() {
  ...
  // if lox.begin failes its becasue it was a warm boot and the VL53LOX is in continues mesurement mode
  // we can use an IO pin to reset the device in case we get stuck in this mode
  while (!lox.begin()) {
  ...
  //Second Parameter options are VL53L0X_GPIOFUNCTIONALITY_OFF, VL53L0X_GPIOFUNCTIONALITY_THRESHOLD_CROSSED_LOW, VL53L0X_GPIOFUNCTIONALITY_THRESHOLD_CROSSED_HIGH, VL53L0X_GPIOFUNCTIONALITY_THRESHOLD_CROSSED_OUT, VL53L0X_GPIOFUNCTIONALITY_NEW_MEASURE_READY

  Serial.println("Set GPIO Config so if range is lower the LowThreshold trigger Gpio Pin ");
  lox.setGpioConfig(VL53L0X_DEVICEMODE_CONTINUOUS_RANGING, VL53L0X_GPIOFUNCTIONALITY_THRESHOLD_CROSSED_LOW, VL53L0X_INTERRUPTPOLARITY_LOW);
...
void VL53LOXISR(){
  ...
  //set the built in LED to reflect in range on for Out of range off for in range
